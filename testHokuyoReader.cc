...
// NOTE: /dev/cu.usbmodem--- should be used here as
// the /dev/tty.usbmodem--- would not responding
#define HOKUYO_DEVICE "/dev/cu.usbmodem1421"
...
int main() {
	...
	if (hokuyoReader.Connect(device, baud)) {
		std::cerr << "testHokuyoReader: could not connect to the sensor."
				<< std::endl;
		exit(-1);
	}

	std::cout << hokuyoReader.GetVendor() << std::endl
			<< hokuyoReader.GetProduct() << std::endl
			<< hokuyoReader.GetFirmware() << std::endl
			<< hokuyoReader.GetProtocol() << std::endl
			<< hokuyoReader.GetSerial() << std::endl;
	...
	if (hokuyoReader.GetScanTypeAndSkipFromName(sensorType, scanName, &newSkip,
			&scanType)) {
		std::cerr
				<< "testHokuyoReader: getScanTypeAndSkipFromName: Error getting the scan parameters."
				<< std::endl;
		exit(-1);
	}
	...
	if (hokuyoReader.SetScanParams(scanName, scanStart, scanEnd, scanSkip,
			encoding, scanType)) {
		std::cerr
				<< "testHokuyoReader: setScanSettings: Error setting the scan parameters."
				<< std::endl;
		exit(-1);
	}
	...
	if (numOutputArgs < 0) {
		std::cerr << "testHokuyoReader: bad number of output args.."
				<< std::endl;
		return -1;
	}
	...
	for (int i = 0; i < 10; i++) {
		if (hokuyoReader.GetScan(data, n_data)) {
			...
			std::cerr << "testHokuyoReader: iteration #" << i
					<< " : scan succeeded! got " << n_data << " points"
					<< std::endl;
	}
