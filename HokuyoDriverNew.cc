...
int HokuyoDriverNew::Setup() {
	...
	int exp_num_points = ceil(
			(max_angle - min_angle + laser_res) / (scan_skip * laser_res));
...
////////////////////////////////////////////////////////////////////////////////
int HokuyoDriverNew::ProcessMessage(QueuePointer &resp_queue,
...
void HokuyoDriverNew::Main() {
	...
	for (;;) {
		...
		int scan_start = (int) (Conf.min_angle / Conf.resolution * scan_skip
				+ count_zero + 0.49);
		int scan_end = (int) (Conf.max_angle / Conf.resolution * scan_skip
				+ count_zero + 0.49);
		...
		//get the ranges and intensities
			...
			if (sensor_type == HOKUYO_TYPE_URG_04LX) {
				ret = hokuyo.GetScan(laser_data, n_data, scan_start, scan_end,
				HOKUYO_RANGE_INTENSITY_AV_AGC_AV, HOKUYO_3DIGITS,
				...
				if (ret == 0)  //successful scan
					...
					for (unsigned int i = 0; i < Data.ranges_count; i++) {
						...
						Data.intensity[i] =
								(uint8_t) (laser_data[3 * i + 2] / 4); //FIXME: use the range of values better
					}
					Publish(device_addr, PLAYER_MSGTYPE_DATA,
							PLAYER_LASER_DATA_SCAN, &Data);
					delete[] Data.ranges;
				...
				if (ret == 0)  //successful scan
					...
					for (unsigned int i = 0; i < Data.ranges_count; i++) {
						...
						Data.intensity[i] = (uint8_t) (laser_data[2 * i + 1]
								/ 40); //FIXME: use the range of values better
					}
					Publish(device_addr, PLAYER_MSGTYPE_DATA,
							PLAYER_LASER_DATA_SCAN, &Data);
					delete[] Data.ranges;
