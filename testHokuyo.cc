...
// NOTE: the /dev/tty.usbmodem--- would not responding
...
int main() {
	...
	if (hokuyo.GetScanTypeAndSkipFromName(sensorType, scanName, &newSkip,
			&scanType)) {
		std::cerr
	...
	if (hokuyo.GetScan(data, n_data, scanStart, scanEnd, scanSkip, encoding,
			scanType, numScans)) {
		std::cerr << "testHokuyo: scan failed" << std::endl;
		return -1;
	}

	std::cerr << "testHokuyo:scan succeeded! got " << n_data << " points."
			<< std::endl;
